<?php
	...
	if ($_SESSION['penjamin_user'] != "") {
		...
		//Masukkan ke tabel
		$insert = $db->query("insert into tbl_surat_rujukan (jenis, no_daftar, nomr, nama, nama_file) values ('Klinik Asal', '".$htravel[0]['no_daftar']."', '".$htravel[0]['nomr']."', '".$htravel[0]['nama']."', '$nama_file')");		
		...
		for ($k = 0; $k < count($dtravel); $k++) {
			$nos = $nos + 1;
			//$linktxt = $linktxt. "$nos. http://103.157.26.142/pages_penjamin/hd/surat_travel_mobile_print.php?id=".md5($dtravel[$k]['id'])."\n";
			$access_token = 'aa9c607342df5b98f3662f3f3217f2f4c9ec3808';
			$long_url = "http://103.157.26.142/pages_penjamin/hd/surat_travel_mobile_print.php?id=".md5($dtravel[$k]['id']);
			$curlBitly = curl_init();
...
Berikut ini adalah surat Rujukan dari klinik Asal yang bisa Anda Download melalui Link yang ada dbawah ini
