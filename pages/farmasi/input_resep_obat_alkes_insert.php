<?php
	...
	$obt = $db->queryItem("select nama_pelayanan from tbl_tarif where kode_tarif='".$_POST['obat']."' and kode_jns_tarif='8'", 0);
	$resepS = $db->query("select nomr, no_daftar from tbl_resep where no_resep='".$_POST['resep']."'");
	$insert = $db->query("insert into tbl_alkes (no_daftar, nomr, kode_tarif, nama_alkes, tarif) values ('".$resepS[0]['no_daftar']."', '".$resepS[0]['nomr']."', '".$_POST['obat']."', '".$obt."', '".$_POST['tarif']."')", 0);
?>
...
<div id="data_pasien">
	...
	<?php
		...
		$alkes = $db->query("select * from tbl_alkes where no_daftar='".$resepS[0]['no_daftar']."' and nomr='".$resepS[0]['nomr']."' and status_delete='UD'", 0);
		if (count($alkes) > 0) {
	...
	<table id="table-data" class="table table-hover table-nomargin dataTable table-bordered nomargin"> 
		...
		<tr>
			...
			<td class="text-center">
				<a class="btn_no_text btn" style="border-radius: 4px;" title="Delete" href="#" onclick="return window.location = 'pages/farmasi/obat_alkes_delete.php?id=<?php echo $alkes[$i]['id'].'&ids='.$_POST['id']?>';">
					<span class="ui-icon ui-icon-circle-close"></span>
				</a>
			</td> 
		</tr> 
