                        ...
                        <tr height="28">
                            ...
                            <div style="margin-bottom: 4px; margin-top: 4px;">
                            	<select id="nodaftar" name="nodaftar" size="1" style="width: 130px;" onchange="CariPasien(this.value)">
									<option value="">--No. Pendaftaran--</option>
									<?php
										$daft = $db->query("select a.no_daftar, b.nm_pasien as nama_pasien from tbl_pendaftaran a left join tbl_pasien b on b.nomr=a.nomr where a.status_pasien='OPEN' and a.status_delete='UD' and b.status_delete='UD'");
										for ($i = 0; $i < count($daft); $i++) {
											echo '<option value="'.$daft[$i]['no_daftar'].'">'.$daft[$i]['no_daftar'].' - '.$daft[$i]['nama_pasien'].'</option>';
										}
										$daft = $db->query("select a.no_daftar, b.nm_pasien as nama_pasien from tbl_pendaftaran_jamsostek a left join tbl_pasien_jamsostek b on b.id=a.idNomr where a.status_pasien='OPEN' and a.status_delete='UD' and b.status_delete='UD'");
										for ($i = 0; $i < count($daft); $i++) {
											echo '<option value="1JAMSOSTEK'.$daft[$i]['no_daftar'].'">'.$daft[$i]['no_daftar'].' - '.$daft[$i]['nama_pasien'].'</option>';
										}
									?>
								</select>
                            </div></td>
