...
<head>
	...
	<title>Print Pembayaran Asuransi</title>
...
<body>
	...
	?>
	<div align="left">
		<p style="text-align: left; font-size: 14px; font-weight: bold; margin-bottom: 5px;">
		<div style="width: 325px; float: right"><img src="../../images/billing_1.png" /></div><br>
		<div style="width: 500px; float: left; margin-left: 12px; font-weight: bold">KWITANSI PEMBAYARAN ASURANSI<br />No : <?php echo $kasir[0]['no_kwitansi'] ?></div>
		</p>
			...
			<tr height="28">
				<td valign="middle" colspan="2">
					<div class="hastable box box-content nopadding" align="left" style="margin-left: 0; margin-right: 0; width: 100%; margin-top: 0px;">
						<?php
						$admnr = $db->queryItem("select sum(bayar) from tbl_kasir_detail where no_kwitansi='" . $_GET['id'] . "' and kategori='ADMINISTRASI' and payment_to='ASURANSI'", 0);
						if ($admnr > 0) {
						?>
							<table id="table-data" class="table table-hover table-nomargin dataTable table-bordered nomargin">
								...
								<tbody>
									<?php
									$biayaAdmin = 0;
									$adm = $db->query("select * from tbl_kasir_detail where no_kwitansi='" . $_GET['id'] . "' and kategori='ADMINISTRASI' and payment_to='ASURANSI'");
						...
						$admnr = $db->queryItem("select sum(bayar) from tbl_kasir_detail where no_kwitansi='" . $_GET['id'] . "' and kategori='PHARMACY' and payment_to='ASURANSI'");
						if ($admnr > 0 and $tst[0] != 'JAM') {
						?>
							<table id="table-data" class="table table-hover table-nomargin dataTable table-bordered nomargin">
								<thead>
									<tr>
										<th colspan="3" style="text-align: left"><span id="openPharmacy" onclick="openPharmacy()">Pharmacy</span> <span id="closePharmacy" style="display: none;" onclick="closePharmacy()">Pharmacy</span></th>
									</tr>
								</thead>
								<tbody id="bodyPharmacy" style="display: none;">
									...
									?>
										...
										?>
											<tr>
												<td style="width: 15px; text-align: right; font-size: 10px;">-</td>
												<td style="text-align: left; font-size: 10px;"><?php echo $farm2[0]['nama'] ?></td>
									...
									$totFarm = 0;
									$adm = $db->query("select * from tbl_kasir_detail where no_kwitansi='" . $_GET['id'] . "' and kategori='PHARMACY' and payment_to='ASURANSI'");
									for ($i = 0; $i < count($adm); $i++) {
									?>
										<tr>
											<td style="width: 15px; text-align: right; font-size: 10px;">-</td>
											<td style="text-align: left; font-size: 10px;"><?php echo $adm[$i]['nama'] ?> - <?php echo $adm[$i]['qty'] ?> <?php echo $adm[$i]['satuan'] ?></td>
						...
						$tindnr = $db->queryItem("select sum(bayar) from tbl_kasir_detail where no_kwitansi='" . $_GET['id'] . "' and kategori='TINDAKAN' and payment_to='ASURANSI'");
						if ($tindnr > 0) {
						?>
							<table width="1052" id="sort-table" style="margin-bottom: 5px; margin-top: 7px;">
								<thead>
									<tr>
								...
								<tbody>
									<?php
									$totTdk = 0;
									$adm = $db->query("select * from tbl_kasir_detail where no_kwitansi='" . $_GET['id'] . "' and kategori='TINDAKAN' and payment_to='ASURANSI'");
									for ($i = 0; $i < count($adm); $i++) {
									?>
										<tr>
											<td style="width: 15px; text-align: right; font-size: 10px;">-</td>
						...
						$alkesnr = $db->queryItem("select sum(bayar) from tbl_kasir_detail where no_kwitansi='" . $_GET['id'] . "' and kategori='alkes' and payment_to='ASURANSI'");
						if ($alkesnr > 0) {
						?>
							<table width="1054" id="sort-table" style="margin-bottom: 5px; margin-top: 7px;">
								...
								<tbody>
									<?php
									$totAlkes = 0;
									$adm = $db->query("select * from tbl_kasir_detail where no_kwitansi='" . $_GET['id'] . "' and kategori='alkes' and payment_to='ASURANSI'");
									for ($i = 0; $i < count($adm); $i++) {
									?>
										<tr>
											<td style="width: 15px; text-align: right; font-size: 10px;">-</td>
						...
						$labnr = $db->queryItem("select sum(bayar) from tbl_kasir_detail where no_kwitansi='" . $_GET['id'] . "' and kategori='LAB' and payment_to='ASURANSI'");
						if ($labnr > 0) {
						?>
							<table id="table-data" class="table table-hover table-nomargin dataTable table-bordered nomargin">
								<thead>
								...
								<tbody>
									<?php
									$totLab = 0;
									$adm = $db->query("select * from tbl_kasir_detail where no_kwitansi='" . $_GET['id'] . "' and kategori='LAB' and payment_to='ASURANSI'");
									for ($i = 0; $i < count($adm); $i++) {
									?>
										<tr>
											<td style="width: 15px; text-align: right; font-size: 10px;">-</td>
						...
						$gigi_nr = $db->queryItem("select sum(bayar) from tbl_kasir_detail where no_kwitansi='" . $_GET['id'] . "' and kategori='GIGI' and payment_to='ASURANSI'");
						if ($gigi_nr > 0) {
						?>
							...
							<table id="table-data" class="table table-hover table-nomargin dataTable table-bordered nomargin">
								...
								<tbody>
									<?php
									$totgigi = 0;
									$adm = $db->query("select * from tbl_kasir_detail where no_kwitansi='" . $_GET['id'] . "' and kategori='GIGI' and payment_to='ASURANSI'");
									for ($i = 0; $i < count($adm); $i++) {
									?>
										<tr>
											<td style="width: 15px; text-align: right; font-size: 10px;">-</td>
						...
						$obygn_nr = $db->queryItem("select sum(bayar) from tbl_kasir_detail where no_kwitansi='" . $_GET['id'] . "' and kategori='OBYGN' and payment_to='ASURANSI'");
						if ($obygn_nr > 0) {
						?>
							<table id="table-data" class="table table-hover table-nomargin dataTable table-bordered nomargin">
								<thead>
								...
								<tbody>
									<?php
									$totObygn = 0;
									$adm = $db->query("select * from tbl_kasir_detail where no_kwitansi='" . $_GET['id'] . "' and kategori='OBYGN' and payment_to='ASURANSI'");
									for ($i = 0; $i < count($adm); $i++) {
									?>
										<tr>
											<td style="width: 15px; text-align: right; font-size: 10px;">-</td>
						...
						$bedah_nr = $db->queryItem("select sum(bayar) from tbl_kasir_detail where no_kwitansi='" . $_GET['id'] . "' and kategori='BEDAH' and payment_to='ASURANSI'");
						if ($bedah_nr > 0) {
						?>
							<table id="table-data" class="table table-hover table-nomargin dataTable table-bordered nomargin">
								<thead>
								...
								<tbody>
									<?php
									$totObygn = 0;
									$adm = $db->query("select * from tbl_kasir_detail where no_kwitansi='" . $_GET['id'] . "' and kategori='BEDAH' and payment_to='ASURANSI'");
									for ($i = 0; $i < count($adm); $i++) {
									?>
										<tr>
											<td style="width: 15px; text-align: right; font-size: 10px;">-</td>
							...
							?>
								<table id="table-data" class="table table-hover table-nomargin dataTable table-bordered nomargin">
									...
									<tbody>
										<?php
										$totRad = 0;
										$adm = $db->query("select * from tbl_kasir_detail where no_kwitansi='" . $_GET['id'] . "' and kategori='RAD' and payment_to='ASURANSI'");
										for ($i = 0; $i < count($adm); $i++) {
										?>
											<tr>
												<td style="width: 15px; text-align: right; font-size: 10px;">-</td>
						...
						$fisnr = $db->queryItem("select sum(bayar) from tbl_kasir_detail where no_kwitansi='" . $_GET['id'] . "' and kategori='FIS' and payment_to='ASURANSI'");
						if ($fisnr > 0) {
							?>
								<table id="table-data" class="table table-hover table-nomargin dataTable table-bordered nomargin">
									<thead>
									...
									<tbody>
										<?php
										$totFis = 0;
										$adm = $db->query("select * from tbl_kasir_detail where no_kwitansi='" . $_GET['id'] . "' and kategori='FIS' and payment_to='ASURANSI'");
										for ($i = 0; $i < count($adm); $i++) {
										?>
											<tr>
												<td style="width: 15px; text-align: right; font-size: 10px;">-</td>
						...
						$prwtnr = $db->queryItem("select sum(bayar) from tbl_kasir_detail where no_kwitansi='" . $_GET['id'] . "' and kategori='PRWT' and payment_to='ASURANSI'");
						if ($prwtnr > 0) {
							?>
								<table id="table-data" class="table table-hover table-nomargin dataTable table-bordered nomargin">
									<thead>
									...
									<tbody>
										<?php
										$totFis = 0;
										$adm = $db->query("select * from tbl_kasir_detail where no_kwitansi='" . $_GET['id'] . "' and kategori='PRWT' and payment_to='ASURANSI'");
										for ($i = 0; $i < count($adm); $i++) {
										?>
											<tr>
												<td style="width: 15px; text-align: right; font-size: 10px;">-</td>
						...
						$ranapnr = $db->queryItem("select sum(bayar) from tbl_kasir_detail where no_kwitansi='" . $_GET['id'] . "' and kategori='RANAP' and payment_to='ASURANSI'");
						if ($ranapnr > 0) {
							?>
								<table id="table-data" class="table table-hover table-nomargin dataTable table-bordered nomargin">
									<thead>
									...
									<tbody>
										<?php
										$totRanap = 0;
										$adm = $db->query("select * from tbl_kasir_detail where no_kwitansi='" . $_GET['id'] . "' and kategori='RANAP' and payment_to='ASURANSI'");
										for ($i = 0; $i < count($adm); $i++) {
										?>
											<tr>
												<td style="width: 15px; text-align: right; font-size: 10px;">-</td>
