<?php
	...
	if ($_SESSION['rg_user'] != '') {
		...
		$insert = $db->query("insert into tbl_fisio_detail (fisioID, nomr, nama_pasien, no_fisio, kode_tarif, tarif, nama_tindakan, qty, tarif_dasar) values ('".$_POST['id']."', '".$_POST['nomr']."', '".$_POST['nama']."', '".$_POST['no_fisio']."', '".$_POST['fis']."', '".$total_tarif."', '$nama_tindakan', '".$_POST['qty']."', '".$_POST['tarif']."')", 0);
		...
		//simpan ke jurnal
		$dataRawat = $db->query("select nomr, no_daftar from tbl_fisio where id='".$_POST['id']."'");
		...
		$keterangan = 'Patient Transaction:  No. Reg.'.$dataRawat[0]['no_daftar'].', Tindakan Fisioterapi: ['.$dokter[0]['nama_dokter'].'] PS: '.$pasien[0]['nm_pasien'];
		$deskripsi = 'Tidakan Fisioterapi ['.$nama_tindakan.'] PS: '.$pasien[0]['nm_pasien'];
		...
		$cost_center_kode = $dtarif[0]['kd_profit'];
		$cost_center_nama = $dtarif[0]['nm_profit'];
		$coa = $db->query("select kd_coa, nm_coa from tbl_coa where kd_coa='41010515'");
		$gl_kode = $coa[0]['kd_coa'];
		$gl_nama = $coa[0]['nm_coa'];
		$debit = 0;
		$kredit = $total_tarif;
