<?php
	...
	if ($_SESSION['rg_user'] != '') {
		...
		$insert = $db->query("insert into tbl_lab_detail (labID, nomr, nama_pasien, no_lab, kode_tarif, tarif, nama_tindakan, qty, tarif_qty, analisis) values ('".$_POST['id']."', '".$_POST['nomr']."', '".$_POST['nama']."', '".$_POST['no_lab']."', '".$_POST['lab']."', '".$tarif_qty."', '$nama_tindakan', '".$_POST['qty']."', '".$_POST['tarif']."', '".$_POST['analisis']."')", 0);
		...
		$keterangan = 'Patient Transaction:  No. Reg.'.$dataRawat[0]['no_daftar'].', Tindakan Laboratorium: ['.$dokter[0]['nama_dokter'].'] PS: '.$pasien[0]['nm_pasien'];
		$deskripsi = 'Tidakan Laboratorium ['.$nama_tindakan.'] PS: '.$pasien[0]['nm_pasien'];
		...
		$cost_center_kode = $dtarif[0]['kd_profit'];
		$cost_center_nama = $dtarif[0]['nm_profit'];
		$coa = $db->query("select kd_coa, nm_coa from tbl_coa where kd_coa='41010509'");
		$gl_kode = $coa[0]['kd_coa'];
		$gl_nama = $coa[0]['nm_coa'];
		$debit = 0;
		$kredit = $tarif_qty;
