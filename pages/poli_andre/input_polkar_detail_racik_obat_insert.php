<?php
	...
	$obt = $db->query("select nama_obat, harga_beli, satuan_terkecil from tbl_obat where kode_obat='".$_POST['obat']."'");
	$total = $obt[0]['harga_beli'] * $_POST['qty'];
	$insert = $db->query("insert into tbl_polkar_detail (racikan_id, polkar_id, no_polkar, kode_obat, nama_obat, qty, harga, total, jenis, satuan) values ('".$_POST['racikan']."', '".$_POST['id']."', '".$_POST['resep']."', '".$_POST['obat']."', '".$obt[0]['nama_obat']."', '".$_POST['qty']."', '".$obt[0]['harga_beli']."', '".$total."', 'R', '".$obt[0]['satuan_terkecil']."')", 0);
?>
...
<div id="data_pasien">
	...
	<table id="table-data" class="table table-hover table-nomargin dataTable table-bordered nomargin"> 
		...
		<tbody> 
		<?php
			 $data = $db->query("select * from  tbl_polkar_detail where status_delete='UD' and jenis='NR' and polkar_id='".$_POST['id']."'", 0);
	...
	<?php
		$resep = $db->query("select * from  tbl_polkar_racikan where no_polkar='".$_POST['resep']."' and status_delete='UD'", 0);
