<div>
    <div class="breadcrumbs">
        <ul>
            ...
            <li>
                <a href="javascript:void(0)">Obat Gudang </a>
            </li>
        </ul>
    </div>
    <div class="row">
                                    ...
                                    <table id="table-data" class="table table-hover table-responsive table-nomargin dataTable table-bordered">
                                        ...
                                        <tr>
                                            ...
                                            for ($i = 0; $i < count($bln); $i++) {
                                                echo '<th style="width:50px">'.$bln[$i].'</th> ';
                                            }
                                            ?>
                                        </tr>
                                        </thead>
                                        ...
                                        if ($_POST['search'] == "")
                                            $data = $db->query("select a.kode_obat, a.nama_obat, a.tahun from tbl_stock_awal a group by a.tahun, a.kode_obat order by a.nama_obat", 0);
                                        else
                                            $data = $db->query("select a.kode_obat, a.nama_obat, a.tahun from tbl_stock_awal a where a.nama_obat like '%".$_POST['search']."%' group by a.tahun, a.kode_obat order by a.nama_obat desc", 0);
                                        for ($i = 0; $i < count($data); $i++) {
                                            ...
                                            <tr>
                                                ...
                                                for ($j = 0; $j < count($bln); $j++) {
                                                    $blnKe = $j + 1;
                                                    $nilai_stok = $db->queryItem("select nilai from tbl_stock_awal where tahun='".$data[$i]['tahun']."' and bulan='".$blnKe."' and kode_obat='".$data[$i]['kode_obat']."'", 0);
                                                    if ($nilai_stok > 0) $nilai_stok_txt = number_format($nilai_stok);
                                                    else $nilai_stok_txt = '-';
                                                    echo '<td style="text-align: right">'.$nilai_stok_txt.'</td>';
                                                }
                                                ?>
                                            </tr>
                                            <?php
