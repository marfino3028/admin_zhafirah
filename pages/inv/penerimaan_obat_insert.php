...
$select = $db->query("select * from tbl_po limit 1", 0);
...
if ($_SESSION['rg_user'] != '') {
	...
	if ($_POST['jenis_penerimaan'] == 'po') {
		...
		$rate = 1;
		$supplier = '';
		$keterangan = 'GRN,  No. Penerimaan.'.$_POST['no_penerimaan'].', Supplier Name: '.$po[0]['nama_suplier'];
		...
		for ($i = 0; $i < count($_POST['kd_obat']); $i++) {
			...
			$insert = $db->query("insert into tbl_penerimaan_detail (penerimaan_id, no_penerimaan, kode_obat, nama_obat, qty, harga_beli, total, margin_obat, harga_jual) values ('" . $id . "', '" . $_POST['no_penerimaan'] . "', '" . $_POST['kd_obat'][$i] . "', '" . $_POST['nama_obat'][$i] . "', '" . $_POST['qty'][$i] . "', '" . $_POST['harga'][$i] . "', '" . $total . "', '" . $_POST['margin'][$i] . "', '" . $harga_jual . "')", 0);

			$gl_kode = '11040101';
			$gl_nama = 'PERSEDIAAN MEDIS OBAT';
			$deskripsi = 'GRN PO#'.$no_po.' FAKTUR#'.$_POST['no_faktur'].' BRG#OBT'.$_POST['kd_obat'][$i].' '.$obat.' '.$_POST['qty'][$i].' @'.$_POST['harga'][$i];
		...
		$update = $db->query("update tbl_penerimaan set jml_obat='$jumlah' where id='$id'");

		$gl_kode = '21010600';
		$gl_nama = 'HUTANG USAHA BELUM DIAKUI';
		$deskripsi = 'GRN PO#'.$no_po.' FAKTUR#'.$_POST['no_faktur'];
		$dtarif = $db->query("select * from tbl_poli where kd_poli='".$_POST['kd_poli']."'");
		$reg_no = '';
		$cost_center_kode = '';
		$cost_center_nama = '';
