                ...
                <div class="box-content nopadding" style="overflow-x:auto;">
					<table id="table-data" class="table table-hover table-responsive table-nomargin dataTable table-bordered">
						...
						<?php
							...
							$data = $db->query("select a.no_obygn, a.nomr, a.nama, a.id, a.total_harga_obygn from tbl_obygn a where a.status_delete='UD' and tgl_input_obygn >= '$date1' and tgl_input_obygn < '$date2'", 0);
							for ($i = 0; $i < count($data); $i++) {
								$no = $start + $i + 1;
								$data[$i]['nama_perusahaan'] = $db->queryItem("select nama_perusahaan from tbl_pendaftaran where nomr='".$data[$i]['nomr']."'");
								$tarif = $db->query("select sum(tarif) as tarif, sum(biaya_dokter) as dokter, sum((tarif-biaya_dokter)) as upk, pDokter from tbl_obygn_detail where obygnID='".$data[$i]['id']."' group by nomr");
						...
						<?php
							...
							if ($ttlDkt > 0) {
								$cash = $db->queryItem("select count(a.id) from tbl_obygn a left join tbl_kasir b on b.no_daftar=a.no_daftar where a.tgl_input_obygn >= '$date1' and a.tgl_input_obygn < '$date2' and a.status_delete='UD' and b.kode_perusahaan='PPP031'", 0);
								$asuransi = $db->queryItem("select count(a.id) from tbl_obygn a left join tbl_kasir b on b.no_daftar=a.no_daftar where a.tgl_input_obygn >= '$date1' and a.tgl_input_obygn < '$date2' and a.status_delete='UD' and b.kode_perusahaan <> 'PPP031' and b.kode_perusahaan <> 'JJJ030'", 0);
								$jamsostek = $db->queryItem("select count(a.id) from tbl_obygn a left join tbl_kasir b on b.no_daftar=a.no_daftar where a.tgl_input_obygn >= '$date1' and a.tgl_input_obygn < '$date2' and a.status_delete='UD' and b.kode_perusahaan='JJJ030'", 0);
						?>
						<tr>
