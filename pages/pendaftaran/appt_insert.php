<?php
	...
	if ($_SESSION['rg_user'] != '') {
		...
		$pasien = $db->query("select * from tbl_pasien where nomr='".$_POST['nomr']."'");
		...
		//print_r($_POST);
		//print_r($pasien);
		if ($pasien[0]['tgl_lahir'] == "0000-00-00") {
		        $pasien[0]['tgl_lahir'] = date("Y-m-d");
			$insert = $db->query("insert into  tbl_perjanjian (no_perjanjian, nomr, kd_poli, nama_poli, kode_perusahaan, nama_perusahaan, status_pasien, kode_dokter, nama_dokter, tgl_daftar, biayaAdmin, tgl_update, status_delete, status_hub, nomr_hub, kelas_id, ruang_id, bed_id, nama_bed, yang_berobat, tgl_insert, status_dokter, nama, nik, tempat_lahir, tanggal_lahir, alamat, jenis_kelamin, email, hp, user_input, mesinHD_id, mesinHD_nama, shift) values ('".$_POST['no_daftar']."', '".$_POST['nomr']."', '".$_POST['kd_poli']."', '".$poli."', '".$_POST['kode_perusahaan']."', '".$perusahaan."', 'OPEN', '".$_POST['kd_dokter']."', '$dokter', '".$_POST['tgl_appt']."', 0, '".date("Y-m-d H:i:s")."', 'UD', '".$_POST['hub']."', '".$_POST['hub_nomr']."', '".$bed[0]['kelas_id']."', '".$bed[0]['kelas_ruang_id']."', '".$bed[0]['id']."', '".$bed[0]['nama']."', '".$_POST['keluarga']."', '".date("Y-m-d H:i:s")."', '".$_POST['kd_oncall']."', '".$pasien[0]['nm_pasien']."', '".$pasien[0]['no_ktp']."', '".$pasien[0]['tmpt_lahir']."', '".$pasien[0]['tgl_lahir']."', '".$pasien[0]['alamat_pasien']."', '".$pasien[0]['jk']."', '".$pasien[0]['email']."', '".$pasien[0]['telp_pasien']."', '".$_SESSION['rg_nama']."', '".$mesin[0]['id']."', '".$mesin[0]['merk_mesin']."', '".$_POST['shift_hd']."')", 1);
		}
		else {
			$insert = $db->query("insert into  tbl_perjanjian (no_perjanjian, nomr, kd_poli, nama_poli, kode_perusahaan, nama_perusahaan, status_pasien, kode_dokter, nama_dokter, tgl_daftar, biayaAdmin, tgl_update, status_delete, status_hub, nomr_hub, kelas_id, ruang_id, bed_id, nama_bed, yang_berobat, tgl_insert, status_dokter, nama, nik, tempat_lahir, tanggal_lahir, alamat, jenis_kelamin, email, hp, user_input, mesinHD_id, mesinHD_nama, shift) values ('".$_POST['no_daftar']."', '".$_POST['nomr']."', '".$_POST['kd_poli']."', '".$poli."', '".$_POST['kode_perusahaan']."', '".$perusahaan."', 'OPEN', '".$_POST['kd_dokter']."', '$dokter', '".$_POST['tgl_appt']."', 0, '".date("Y-m-d H:i:s")."', 'UD', '".$_POST['hub']."', '".$_POST['hub_nomr']."', '".$bed[0]['kelas_id']."', '".$bed[0]['kelas_ruang_id']."', '".$bed[0]['id']."', '".$bed[0]['nama']."', '".$_POST['keluarga']."', '".date("Y-m-d H:i:s")."', '".$_POST['kd_oncall']."', '".$pasien[0]['nm_pasien']."', '".$pasien[0]['no_ktp']."', '".$pasien[0]['tmpt_lahir']."', '".$pasien[0]['tgl_lahir']."', '".$pasien[0]['alamat_pasien']."', '".$pasien[0]['jk']."', '".$pasien[0]['email']."', '".$pasien[0]['telp_pasien']."', '".$_SESSION['rg_nama']."', '".$mesin[0]['id']."', '".$mesin[0]['merk_mesin']."', '".$_POST['shift_hd']."')", 1);
		}
...
Atas Nama : '.$pasien[0]['nm_pasien'].'
Alamat : '.$pasien[0]['alamat_pasien'].'
Tempat & Tgl Lahir : '.$pasien[0]['tmpt_lahir'].', '.date("d F Y", strtotime($pasien[0]['tgl_lahir'])).'
Tujuan / Layanan : '.$poli.'
