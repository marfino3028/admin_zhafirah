<script>
    ...
    document.getElementById('noSep').value = noSep;
    ...
    function terbitkanRujukan() {
        ...
        var jenis_query = "Create Rujukan";
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.metaData.code == 200) {
                    Swal.fire({
                        title: 'Berhasil',
                        text: 'No.Rujukan : ' + data.response.rujukan.noRujukan,
                        icon: 'success'
                    }).then((result) => {
                        printRujukan(data.response.rujukan.noRujukan)
