...
// Ambil daftar paket haji yang masih aktif dan belum penuh
$packages = $db->query("
    SELECT 
        p.id, 
        p.name, 
    ...
    FROM packages p
    WHERE p.package_type = 'haji' AND p.status = 'active'
");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_booking'])) {
    ...
    // Format: ZHJ-YYYYMMDD-USERID-PACKAGEID
    $booking_code = "ZHJ-" . date("Ymd") . "-" . $user_id . "-" . $package_id;
    ...
    if ($existing_booking) {
        ...
        if ($insert) {
            echo "<script>alert('Pendaftaran baru berhasil ditambahkan.'); window.location.href='?page=pendaftaran/haji';</script>";
        } else {
...
<div class="form-section">
    ...
    <form method="POST" action="">
        ...
        <a href="?page=pendaftaran/haji" style="margin-left: 10px;">Batal</a>
    </form>
</div>
