<?php
	...
	if ($_SESSION['rg_user'] != '') {
		...
		$insert = $db->query("insert into tbl_po (no_po, no_ro, tgl_input_po, unit, input_by, kode_vendor, nama_vendor, kode_suplier, nama_suplier) values ('".$_POST['no_po']."', '".$_POST['no_ro']."', '".date("Y-m-d")."', '".$_POST['unit']."', '".$_SESSION['rg_user']."', '".$_POST['vendor']."', '$nama_vendor', '".$_POST['suplier']."', '$nama_suplier')", 0);
		$id = mysql_insert_id();
		...
		$update = $db->query("update tbl_ro set status_po='PROSES' where no_ro='".$_POST['no_ro']."'");
		$update = $db->query("update tbl_po set total_po='$jml_po' where id='".$id."'");
