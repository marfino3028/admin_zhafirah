<?php
	...
	if ($_SESSION['rg_user'] != '') {
		...
		$insert = $db->query("insert into tbl_ro_to_gudang_detail (ro_gudangID, no_ro_gudang, kode_obat, nama_obat, sat, qty, harga, total, bln1, bln2, bln3, rata, stok_akhir) values ('".$_POST['id']."', '".$_POST['no_ro']."', '".$_POST['obat']."', '".$data[0]['nama_obat']."', '".$data[0]['satuan_besar']."', '".$_POST['qty']."', '".$data[0]['harga_beli']."', '".$total."', '".$bln1."', '".$bln2."', '".$bln3."', '".$rata."', '".$data[0]['stock_akhir']."')", 0);
		$id = mysql_insert_id();
		
		$jmlRequest = $db->queryItem("select sum(qty) from tbl_ro_to_gudang_detail where no_ro_gudang='".$_POST['no_ro']."'");
		$update = $db->query("update tbl_ro_to_gudang set total_permintaan='$jmlRequest' where no_ro_gudang='".$_POST['no_ro']."'");
	}
?>
<div id="data_pasien">
  <table id="table-data" class="table table-hover table-nomargin dataTable table-bordered nomargin">
	...
	<tbody>
	  <?php
			$data = $db->query("select * from tbl_ro_to_gudang_detail where status_delete='UD' and ro_gudangID='".$_POST['id']."'", 0);
	  ...
	  <tr>
		...
		<td class="text-center"><a class="btn_no_text btn" style="border-radius: 4px;" title="Delete" href="#" onclick="return window.location = 'pages/inv/ApotikGudang_detail_delete.php?id=<?php echo $data[$i]['id'].'&subid='.$_POST['id']?>';"> <span class="ui-icon ui-icon-circle-close"></span> </a> </td>
	  </tr>
	  <?php
