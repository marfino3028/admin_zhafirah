                ...
                <div class="box-content nopadding" style="overflow-x:auto;">
                        ...
                        <table id="table-data" class="table table-hover table-responsive table-nomargin dataTable table-bordered">
                            ...
                            <?php
								...
								if ($_POST['search'] == "") 
									$data_nr = $db->queryItem("select count(a.id) from tbl_pendaftaran a where a.status_delete='UD'", 0);
								else 
									$data_nr = $db->queryItem("select count(a.id) from tbl_pendaftaran a left join tbl_pasien b on b.nomr=a.nomr where a.status_delete='UD' and b.status_delete='UD' and b.nm_pasien like '%".$_POST['search']."%'", 0);
								//$nrdata = $data_nr;
								$nrdata = 100;
                                ...
                                if ($_POST['search'] == "") 
									...
									$data = $db->query("select a.nomr, a.kode_dokter, a.id, a.status_pasien, a.nama_perusahaan, a.biayaAdmin, a.kd_poli from tbl_pendaftaran a where a.status_delete='UD' order by a.id desc", 0);
								else
                                	//$data = $db->query("select a.nomr, a.kode_dokter, a.id, a.status_pasien, a.nama_perusahaan, b.nm_pasien as nama, if (c.nama_poli is NULL, a.kd_poli, c.nama_poli) as nama_poli, c.tarif, a.biayaAdmin from tbl_pendaftaran a left join tbl_pasien b on b.nomr=a.nomr left join tbl_poli c on c.kd_poli=a.kd_poli where a.status_delete='UD' and b.nm_pasien like '%".$_POST['search']."%' order by a.id desc", 0);
									$data = $db->query("select a.nomr, a.kode_dokter, a.id, a.status_pasien, a.nama_perusahaan, a.biayaAdmin, a.kd_poli, b.nm_pasien as nama from tbl_pendaftaran a left join tbl_pasien b on b.nomr=a.nomr where a.status_delete='UD' and b.nm_pasien like '%".$_POST['search']."%' order by a.id desc", 0);
