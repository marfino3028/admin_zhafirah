...
<div class="page-title ui-widget-content ui-corner-all">
					<h1>Data Master: <b>Kategori Sub Menu </b></h1>
					<div class="other">
						<div class="float-left">Insert, Update, Delete Data Master Kategori Sub Menu </div>
						<div class="button float-right">

							<a href="index.php?mod=user&submod=kat_menu_sub_new"  class="btn btn-sm btn-small btn-darkblue rounded pull-right"><span class="fa fa-plus-circle"></span> Tambah Data Kategori Sub Menu </a>
						</div>
                ...
                <div class="box-content nopadding" style="overflow-x:auto;">
                        ...
                        <table id="table-data" class="table table-hover table-responsive table-nomargin dataTable table-bordered">
                            ...
                            <?php
								...
								if ($_POST['search'] == "") 
									$data_nr = $db->queryItem("select count(id) from tbl_kat_sub_menu where status_delete='UD'", 0);
								else 
									$data_nr = $db->queryItem("select count(id) from tbl_kat_sub_menu where nm_ka_menu like '%".$_POST['search']."%' and status_delete='UD'", 0);
								$nrdata = $data_nr;
						
								if(!isset($number)) $number=$nrdata;
								$page=new pages();
								if ($_POST['search'] == "")
									$page->link="index.php?mod=user&submod=kat_menu_sub";
								else 
									$page->link="index.php?mod=user&submod=kat_menu_sub&search=".$_POST['search'];
                                ...
                                if ($_POST['search'] == "") 
									$data = $db->query("select * from tbl_kat_sub_menu where status_delete='UD' order by kategori_id", 0);
								else
									$data = $db->query("select * from tbl_kat_sub_menu where nm_ka_menu like '%".$_POST['search']."%' and status_delete='UD' order by kategori_id", 0);
                                for ($i = 0; $i < count($data); $i++) {
									$kategori = $db->queryItem("select nm_ka_menu from tbl_kat_menu where kategori_id='".$data[$i]['kategori_id']."'");
                            ?>
                            <tr>
