<?php
	...
	if ($_SESSION['rg_user'] != '') { 
		...
		echo '<select id="kat_layanan" name="kat_layanan" size="1" class="form-control">';
		echo '<option value="">--Pilih Jenis Layanan '.$jenis.'--</option>';
		for ($i = 0; $i < count($data); $i++) {
			echo '<option value="'.$data[$i]['kode_kat_pelayanan'].'">'.$data[$i]['kode_kat_pelayanan'].' - '.$data[$i]['nama_kat_pelayanan'].'</option>';
		}
		echo '</select>';
	}
?>