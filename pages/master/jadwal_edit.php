<?php
	$jadwal = $db->query("select * from tbl_jadwal where md5(id)='".$_GET['id']."'");
?>
<div>
    <div class="breadcrumbs">
        <ul>
            ...
            <li>
                <a href="javascript:void(0)">Jadwal Dokter</a>
                <i class="fa fa-angle-right"></i>
            </li>
            <li>
                <a href="javascript:void(0)">Edit Jadwal Dokter</a>
            </li>
        </ul>
    </div>
    <div class="row">
                                                        ...
                                                        <select class="form-control" id="poli" name="poli">
                                        					...
                                        					<?php
                                        					    ...
                                        					    for ($i = 0; $i < count($sub); $i++) {
                                        					        if ($jadwal[0]['kd_poli'] == $sub[$i]['kd_poli']) {
                                        					            echo '<option value="'.$sub[$i]['kd_poli'].'" selected>'.$sub[$i]['nama_poli'].'</option>';
                                        					        }
                                        					        else {
                                            ...
                                            <div class="col-sm-4">
                                                        <select class="form-control" id="dokter" name="dokter">
                                        					...
                                        					<?php
                                        					    ...
                                        					    for ($i = 0; $i < count($sub); $i++) {
                                        					        if ($jadwal[0]['kode_dokter'] == $sub[$i]['kode_dokter']) {
                                        					            echo '<option value="'.$sub[$i]['kode_dokter'].'" selected>'.$sub[$i]['nama_dokter'].'</option>';
                                        					        }
                                        					        else {
				...
				<div class="form-group">
					...
					<div class="col-sm-4">
						<select id="tanggal" name="tanggal" class="chosen-select form-control" >
							...
							for ($i = 1; $i < 8; $i++) {
							    if ($jadwal[0]['hari'] == $i) {
								    echo '<option value="'.$i.'" selected>'.$hr[$i].'</option>';
							    }
							    else {
					...
					<div class="col-sm-2">
						<input type="text" id="mulai2" name="mulai2" class="form-control" placeholder="Mulai" value="<?php echo $jadwal[0]['mulai']?>" />
					</div>
					<label for="textfield" class="control-label col-sm-1" style="text-align: right;">Selesai</label>
					<div class="col-sm-2">
						<input type="text" id="selesai2" name="selesai2" class="form-control" placeholder="Selesai" value="<?php echo $jadwal[0]['selesai']?>" />
					</div>
				</div>
				<div class="form-group">
					...
					<div class="col-sm-10">
						<input type="text" id="keterangan" name="keterangan" class="form-control" autocomplete="off" placeholder="Keterangan" value="<?php echo $jadwal[0]['keterangan']?>" />
					</div>
				</div>
				<div class="form-group">
					...
					<div class="col-sm-2">
						<select id="janji" name="janji" class="chosen-select form-control" >
							...
							<?php
							    if ($jadwal[0]['janji'] == '*Dengan Perjanjian') {
							        echo '<option value="*Dengan Perjanjian" selected>Ya</option>';
							    }
							    else {
							        echo '<option value="*Dengan Perjanjian">Ya</option>';
							    }
							?>
						</select>
					</div>
                    	...
                    	<small>
                        	Apabila pilih Ya, maka jadwalnya adalah : <i>Dengan Perjanjian</i> dan kolom mulai dan selesai akan kosong
                                        ...
                                        <div class="form-actions">
                                            <input type="hidden" name="id" value="<?php echo md5($jadwal[0]['id'])?>" >
                                            <input type="button" name="simpan" id="simpan" class="btn btn-sm btn-small btn-primary rounded" value="Update Jadwal Dokter" onclick="simpanData(this.form, 'pages/master/jadwal_update.php')" />
                                            <input type="button" name="simpan" id="simpan" class="btn btn-sm btn-small btn-danger rounded" value="Hapus Jadwal Dokter" onclick="simpanData(this.form, 'pages/master/jadwal_delete.php')" />
                                        </div>
                                    </form>
                                </div>
                            </div>
